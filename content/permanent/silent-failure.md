...
Silent failures are particularly dangerous in multi-agent systems where dependencies are complex. They stand in direct contrast to **Transparent Failures**, where a [[subagent-definition|sub-agent]] returns a structured, actionable error. A transparent failure provides the [[orchestrator|orchestrator]] with the necessary context (error type, description) to make an informed decision, such as retrying the task, escalating to another agent, or halting the workflow gracefully.
