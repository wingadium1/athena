...
- L1: Agents, Workflows, and Chatbots — What's Actually Different
  This lecture explores the distinctions between agents, workflows, and chatbots, emphasizing the definition of [[permanent/agentic-systems|agentic systems]]. Here are the main points:
  1. Agentic Systems: These systems autonomously pursue goals by perceiving context, selecting actions, invoking tools, and iterating until completion. This contrasts with [[permanent/deterministic-workflows|deterministic workflows]], which are predefined and do not allow for decision-making.
  2. Limitations of [[permanent/chatbots|Chatbots]]: While chatbots seem powerful, they are fundamentally stateless. They can generate responses but do not take actions in the real world.
  3. Transition Necessity: Transitioning from a chatbot to an agentic system relies on effective [[permanent/tool-use|tool use]], necessitating a mechanism to decide when and how tools are utilized.
  ...
  5. [[permanent/action-environment-gap|Action Environment Gap]]: This represents the difference between what the model can describe and what it can actually execute, underscoring the need for a runtime layer to handle structured outputs.
  ...
  8. [[permanent/ccaf-framework|CCAF Framework]]: This framework is introduced as a method to mitigate risks associated with agentic systems.
...
- L2: Tool Use as the Foundation of Agency
  This lecture focuses on [[permanent/tool-use|tool use]] as a key aspect of agency in language models (LLMs). Here are the main points:
  ...
  2. [[permanent/four-phase-lifecycle-of-a-tool-call|Four-Phase Lifecycle of a Tool Call]]:
...
- L3: Perception, Reasoning, Action, and Observation
  This lecture discusses the [[permanent/agentic-loop|agentic loop]], which is crucial for the functioning of agents and consists of four interconnected phases: perception, reasoning, action, and observation. Here are the main points:
  ...
  2. Key Concepts:
    * [[permanent/tool-use|Tool Use]]: An essential aspect of the action phase, where the model makes structured calls to external tools.
    * [[permanent/dynamic-planning|Dynamic Planning]]: The model's capability to adjust its plans based on new information and changing circumstances.
  ...
  4. [[permanent/context-capacity|Context Capacity]]: The lecture explains the importance of the context window, which fills with inputs and results over time. Efficient management of this memory is crucial for effective agent design.
...
- L4: Loop Control — Stopping, Iterating, and Escalating
  ...
  1. [[permanent/termination-conditions|Termination Conditions]]: These are explicit criteria that indicate when an agent should end its loop, critical for preventing infinite loops.
  2. [[permanent/turn-budgets|Turn Budgets]]: This is a strict limit on the number of iterations an agent can execute. Setting a reasonable maximum is important to mitigate risks of continual looping.
  3. [[permanent/escalation|Escalation]]: This involves passing the problem on to a human operator or another fallback system when the agent is unable to progress further.
  4. Reasons for Stopping the Loop:
    * [[permanent/task-completion|Task Completion]]: The goal has been achieved.
    * [[permanent/error-threshold|Error Threshold]]: An error limit has been reached.
    * Exhausting the [[permanent/turn-budgets|Turn Budget]]: The agent has hit its iteration limit.
    * [[permanent/stop-signal|Stop Signal]]: A command from the orchestrator or user indicates to cease operations.
  5. Identifying Failures: [[permanent/infinite-loops|Infinite loops]] can stem from a lack of stop conditions, ongoing errors, or circular reasoning. Implementing termination conditions is vital to address these issues.
  6. [[permanent/progress-vs-spinning|Progress vs. Spinning]]: Progress is marked by new observations and forward movement towards goals. In contrast, spinning consists of repetitive actions that yield no meaningful results. The lecture suggests techniques for detecting spinning, such as comparing successive observations.
...
- L5: When Workflows Outperform Agents
  ...
  1. Definitions:
      * [[permanent/deterministic-workflows|Deterministic Workflows]]: Fixed sequences of steps designed by developers, providing predictability and control.
      * [[permanent/agentic-systems|Agentic Systems]]: Allow dynamic decision-making by the model, which can lead to unpredictability.
      * 
  ...
  5. [[permanent/workflow-patterns|Workflow Patterns]]: Five core patterns of workflows are outlined:
      * [[permanent/prompt-chaining|Prompt Chaining]] -> simplest workflow pattern, passing one call’s output directly as input to the next -> linear, testable, easy to debug
      * [[permanent/routing|Routing]]
      * [[permanent/parallelization|Parallelization]]
      * [[permanent/orchestrator-sub-agent|Orchestrator Sub-Agent]]
      * [[permanent/evaluator-optimizer|Evaluator Optimizer]] These maintain predictability and auditability without the unpredictability of agents.
...
- L6: Choosing the Right Architecture
  ...
  2. Framework vocabulary
      1. [[permanent/incremental-complexity|Incremental Complexity]]: Emphasizes starting with the simplest architecture and only adding complexity when necessary to avoid over-engineering.
      2. [[permanent/role-of-the-orchestrator|Role of the Orchestrator]]: An orchestrator manages tasks and synthesizes results, playing a crucial role in coordinating workflows. -> decompress a high-level goal, assign subtasks to subagents or tools, and synthesizes final results
      3. Risks of [[permanent/over-engineering|Over-Engineering]]: The lecture warns against introducing unnecessary complexity, which can lead to increased costs and risks without proportional benefits. -> adding architecture complexity (e.g: agentic autonomy, introduces cost and risk without delivering proportional benefits for the tasks)
  ...
  4. Signals for Agentic Complexity: It identifies three conditions that indicate when agentic complexity is warranted:
      * [[permanent/open-ended-goals|Open-ended goals]] (and cannot be fully pre-spocified) -> know the target but do not know the step at design time
      * [[permanent/unpredictable-tool-sequences|Unpredictable tool sequences]] -> at design time
      * [[permanent/dynamic-replanning|Dynamic replanning]] during execution -> If these conditions are not met, workflows are generally preferred.
